...
length_coeff = 6;
    ...
    function [ value ] = policy_value( x_co, theta, mu,lambda)
        ...
        for i=2:n
            ...
            emi_data(i,2)=emi_data(i-1,2)*0.25+ x_co*0.8*a + psi(i,2);
            emi_data(i,3)=emi_data(i-1,3)*0.5+x_co*0.05*emi_data(i-1,3)*a+x_co*0.5*a+psi(i,3);
        end
    ...
    function [  sat  ] = ineq_test(x_co, theta)
...
%fixed lambda
    function [sat, theta] = solve_theta(lambda)
        f=@(theta) policy_value( x_co,theta, mu_star, lambda);
...
f_star=@(theta) policy_value( x_co, theta, mu_star, lambda_star);
...
txt_name=strcat('optimal_burden_', num2str(tau),'_',num2str(p_explore),'_',num2str(p_s),'x_coeff',num2str(x_coeff(j)),'.txt');
id=fopen(txt_name,'w');
